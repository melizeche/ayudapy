...
class HelpRequest(models.Model):
    ...
    message = models.TextField(
        ...
        help_text=mark_safe(
            _(
                "Here you can tell in detail what you need, <b> the better you tell your situation the more likely they want to help you </b>"
            )
        ),
        max_length=2000,
        null=True,
    ...
    address = models.CharField(
        _("Address"),
        help_text=_(
            "Your address, city, neighborhood, references, or how to get there, to get help"
        ),
        max_length=400,
        blank=False,
        null=True,
    ...
    location = models.PointField(
        ...
        help_text=mark_safe(
            '<p style="margin-bottom:5px;font-size:10px;">{}<br>{} <a href="#" class="is-link modal-button" data-target="#myModal" aria-haspopup="true">{}</a></p><p id="div_direccion" style="font-size: 10px; margin-bottom: 5px;"></p>'.format(
                _(
                    "Select your location so that people can find you, if you do not want to mark your home a good option may be the nearest police station or some other nearby public place."
                ),
    ...
    added = models.DateTimeField(
        _("Added"), auto_now_add=True, null=True, blank=True, db_index=True
    )
...
class Status(models.Model):
    name = models.CharField(
        "Nombre del estado", max_length=40, help_text="Nombre del estado"
    )
...
class Device(models.Model):
    device_iid = models.AutoField(primary_key=True)
    ...
    ua_string = models.CharField(
        "User Agent", max_length=512, help_text="User Agent", null=True, blank=True
    )
    status = models.CharField(
        "Estado", max_length=32, help_text="Estado del Dispositivo", default="ACTIVE"
    )
    ...
    os_family = models.CharField(
        "SO", max_length=128, help_text="Sistema Operativo", null=True, blank=True
    )
    ...
    created = models.DateTimeField(
        "Creado", help_text="Fecha de Creación del Dispositivo", auto_now=True
    )
    last_seen = models.DateTimeField(
        "Última Visita", help_text="Última Visita del Dispositivo", auto_now_add=True
    )
...
class User(models.Model):
    user_iid = models.AutoField(primary_key=True)
    user_type = models.CharField("Tipo", max_length=32, help_text="Tipo de usuario")
    user_value = models.CharField(
        "Sujeto", max_length=128, help_text="Valor/Nombre de Usuario"
    )
    name = models.CharField(
        "Nombre Completo",
        max_length=512,
...
class HelpRequestOwner(models.Model):
    help_request = models.OneToOneField(
        HelpRequest, on_delete=models.CASCADE, primary_key=True
    )
