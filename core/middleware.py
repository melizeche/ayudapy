...
class AyudaPYMiddleware(object):
    ...
    def __call__(self, request):
        request.ayuda_session = {"user": None, "device": None, "must_set_cookie": False}

        try:
            request.ayuda_session["device"] = self.get_or_create_device(request)
            request.ayuda_session["user"] = self.get_or_create_user(
                request, request.ayuda_session["device"]
            )
        except Exception:
        ...
        # setup cookie
        if (
            request.ayuda_session["must_set_cookie"]
            and request.ayuda_session["device"] is not None
        ):
    ...
    def get_or_create_user(self, request, device):
        try:
            ...
            user = User.objects.get(
                user_type=USER_TYPE_DEVICE, user_value=device.device_id
            )
